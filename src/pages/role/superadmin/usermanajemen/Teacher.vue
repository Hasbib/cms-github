...
const filteredData = computed(() => {
    ...
    return sortedData.filter(teacher =>
...
const fetchTeacherData = async () => {
    try {
        ...
        teacherData.value = response.data.filter(user => user.role === 'teacher');
    } catch (error) {
...
const showEditTeacherModal = () => {
    form.value.name = currentTeacher.value.name;
    ...
    <div class="navbg-sa">
        ...
        <div id="contentsa" class="dashboard-sa">
            <div class="container mt-80">
                                ...
                                <table class="table custom-table rounded-4">
                                    <thead class="thead-custom">
                                        <tr class="ps-4">
                                            ...
                                            <th class="fs-16 fw-light w-400">Name Teacher</th>
                ...
                <!-- Edit Modal -->
                <div v-if="isEditModalVisible" class="modal-backdrop" @click="closeEditTeacherModal">
                </div>
                <div v-if="isEditModalVisible" class="modal fade show d-block" role="dialog"
                    aria-labelledby="exampleModalLabel" aria-hidden="false" @click.self="closeEditTeacherModal">
                    <div class="modal-dialog custom-modal modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header mb--3">
                                <h5 class="fs-16 fw-medium" id="exampleModalLabel">
                                    <i class="bi bi-pencil-square me-1"></i>Edit Teacher
                                </h5>
                                <button type="button" class="btn-close fs-12 c-close"
                                    @click="closeEditTeacherModal"></button>
                            </div>
                            <hr class="mt-0">
                            <div class="px-3 mb-2">
                                <div class="d-flex justify-content-between mt-3">
                                    <label for="teacherName" class="me-3 fs-16 mb-0 mt-2">Name</label>
                                    <input type="text" id="teacherName" class="form-control w-75 h-43"
                                        placeholder="Enter teacher name" v-model="form.name" />
                                </div>
                                <div class="d-flex justify-content-between mt-3">
                                    <label for="teacherEmail" class="me-3 fs-16 mb-0 mt-2">Email</label>
                                    <input type="email" id="teacherEmail" class="form-control w-75 h-43"
                                        placeholder="Enter email name" v-model="form.email" />
                                </div>
                                <div class="d-flex justify-content-between mt-3">
                                    <label for="teacherPassword" class="me-3 fs-16 mb-0 mt-2">Password</label>
                                    <input type="password" id="teacherPassword" class="form-control w-75 h-43"
                                        placeholder="Enter password name" v-model="form.password" />
                                </div>
                                <div class="d-flex justify-content-between mt-3">
                                    <label for="teacherStatus" class="me-3 fs-16 mb-0 mt-2">Status</label>
                            ...
                            <div class="d-flex justify-content-center mb-5">
                                <ButtonTransparanComponen
                                    ...
                                    @click="closeEditTeacherModal">Cancel</ButtonTransparanComponen>
                                <ButtonBiru class="ms-3 mt-4 my-0 h-40 w-30 rounded-3 fs-16" @click="saveTeacher">Save
                                </ButtonBiru>
                            </div>
                        </div>
                    </div>
